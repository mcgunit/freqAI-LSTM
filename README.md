...
As an example, to backtest the --timerange 20230301-20240601 using the example config which sets train_period_days to 120, together with startup_candle_count: 20 (found in the strategy file) on a maximum include_timeframes of 8h, the start date for the downloaded data needs to be 120 days - 20 * 8h / 24 hours = 114 days earlier than the start of the desired training time range. So we would use timerange --timerange 20220901-20240601
